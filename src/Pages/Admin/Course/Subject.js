...
import {
  addSubjectMaterialsApi,
  addSubjectOutlineApi,
  getASubjectsApi,
  removeSubjectMaterialsApi,
  removeSubjectOutlineApi,
...
export const Subject = (props) => {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      getAllExamApi({ subjectId: subject._id })
        .then((data) => {
          if (data.error) throw data.message;
          setExam([...data.data]);
        })
  ...
  const handleSubmit = (e) => {
    ...
    createChapterApi({
      ...
      curriculumId: location.state
        ? location.state.subject.curriculumId._id
        : "",
    }).then((data) => {
      console.log(data);
  ...
  if (chapter.length === 0) {
    chapterShow = (
      <div className="p-40 text-center col-span-12">Not chapter found</div>
    );
  } else {
    chapterShow = chapter.map((item, index) => {
      return (
        <Link
          ...
          state={{ chapter: item }}
          className="card  col-span-6 md:col-span-3 glass bg-inherit hover:bg-slate-600 hover:text-white "
        >
  ...
  const removeOutline = (position) => {
    removeSubjectOutlineApi(location.state.subject._id, position).then(
      (data) => {
  ...
  const addOutline = (e) => {
    e.preventDefault();
    addSubjectOutlineApi(location.state.subject._id, updateOutline).then(
      (data) => {
        window.alert(data);
      }
    );
  ...
  const removeMaterial = (position) => {
    removeSubjectMaterialsApi(location.state.subject._id, position).then(
      (data) => {
  ...
  const addMaterial = (e) => {
    e.preventDefault();
    addSubjectMaterialsApi(location.state.subject._id, updateMaterial).then(
      (data) => {
        window.alert(data.data);
      }
    );
  ...
  return (
    <div>
      <div className="my-10 text-2xl text-center font-bold">
        Subject: {location.state ? location.state.subject.subject : ""}
      </div>
      ...
      <button
        onClick={() => document.getElementById("addChapterModal").showModal()}
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row justify-center">
          {outlines.map((item, index) => {
            return (
              <div className="my-5 flex border p-2 shadow bg-slate-100 me-3">
                <div onClick={() => showFile(item)} className="btn btn-outline">
                  {item.name}
                </div>
                <button
